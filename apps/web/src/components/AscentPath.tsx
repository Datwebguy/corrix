 ...
 * Not a generic AI "receipt card". A climbing step-ladder that agents ascend:
 * claim → checks → verdict → sealed hash → settle.
 */
const STEPS = [
  {
    id: "claim",
  ...
  return (
    ...
    >
      <div className="ascent-header">
        ...
        <span className="ascent-hint">{paused ? "Hover a rung" : "Climbing…"}</span>
      </div>
          ...
          {STEPS.map((step, i) => {
            ...
            return (
              <motion.li
                ...
                style={{ ["--shift" as string]: `${(i % 2 === 0 ? -1 : 1) * 10}px` }}
                onMouseEnter={() => {
                  setPaused(true);
                  setActive(i);
                }}
              ...
              >
                <button type="button" className="ascent-rung-btn">
